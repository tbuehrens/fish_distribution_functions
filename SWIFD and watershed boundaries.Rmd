...
Dissolving HUC 12s by HUC10s and filtering (example: coastal steelhead pops...find HUC 10's for each pop):
...
chehalis_huc8<-c('17100104','17100103')
...
sf_swifd_pops<-sf_swifd%>%
  st_join(sf_pops)%>%
  filter(!is.na(population))%>%
  group_by(population,DISTTYPE_DESC)%>%
...
p1<-ggplot() +
  ...
  geom_sf(data = sf_swifd_pops%>%
            filter(!DISTTYPE_DESC %in% c("Gradient Accessible", "Potential","Historic - Documented","Artificial - Potential","Transported - Potential","Modeled"))
          ,aes(color=as.factor(DISTTYPE_DESC)))
